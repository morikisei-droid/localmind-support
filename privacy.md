...
## 1-3. マイク・音声認識・添付ファイル・AIメモリ（すべて端末内処理）

本アプリには、会議や音声メモの録音と文字起こし、画像（OCR）・PDF・テキストファイルの添付、および会話から重要事項を抽出してノートへの追記を提案する「AIメモリ（スクライブ）」機能があります。

- **録音した音声・文字起こしテキスト・添付ファイルとその抽出テキスト（OCR結果を含む）・スクライブが提案／保存するノート内容は、すべて端末内でのみ処理・保存され、外部へ送信されることはありません。**
- 文字起こしは Apple の**オンデバイス音声認識のみ**（requiresOnDeviceRecognition）を使用し、音声データが Apple のサーバーへ送られることはありません。
- マイク・音声認識へのアクセスは iOS のシステム許可を得た場合にのみ行われ、許可は「設定」からいつでも取り消せます。
- スクライブによるノートの追記・変更は、**必ずユーザーが内容を確認して承認した場合にのみ**適用されます。

## 2. 広告 SDK（Google AdMob）が収集・送信する情報

> **現行バージョン（v1.0）について：** v1.0 は広告を表示せず、本章に記載の広告関連の外部送信は一切行われません。本章は、将来のバージョンで広告を導入した場合に適用される内容を事前に開示するものです。広告導入時には、アプリ内およびストア掲載情報でお知らせします。
...
## 1-2. Optional Web Search (Off by Default)

The App includes an optional web search feature. It is **disabled by default** and operates only when you explicitly enable it in Settings (Internet Search). When enabled, **only your search query (your question, or a short search string the model derives from it)** is sent to the selected search provider (default: Wikipedia, operated by the Wikimedia Foundation), together with the IP address and connection metadata inherent to any HTTPS request. Notes, folders, links, RAG content, personas, model files, and the assembled prompt are never sent. Wikipedia search results are provided under CC BY-SA 4.0 and sources are shown in the app; handling is governed by the Wikimedia privacy policy: https://foundation.wikimedia.org/wiki/Policy:Privacy_policy. You can disable the feature at any time; while disabled, no queries are transmitted.

## 1-3. Microphone, Speech Recognition, Attachments, and AI Memory (All On-Device)

The App can record meetings and voice memos with on-device transcription, attach images (OCR), PDFs, and text files to chats, and propose note updates extracted from your conversations ("AI Scribe").

- **Recorded audio, transcripts, attached files and their extracted text (including OCR results), and Scribe-proposed note content are processed and stored exclusively on your device and are never transmitted externally.**
- Transcription uses Apple's **on-device speech recognition only** (requiresOnDeviceRecognition); your audio is never sent to Apple's servers.
- Microphone and speech-recognition access occurs only with your iOS system permission, which you can revoke at any time in Settings.
- Scribe note updates are applied **only after you review and approve them.**

## 2. Information Collected and Transmitted by the Ad SDK (Google AdMob)

> **About the current version (v1.0):** v1.0 displays no ads and performs none of the ad-related transmissions described in this section. This section is advance disclosure for a future version that may introduce advertising; any such change will be announced in the App and in the store listing.
